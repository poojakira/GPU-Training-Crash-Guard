...
def plot_before_after():
    ...
    metrics = {
        "Peak Memory (MB)": (data["baseline"]["peak_memory_mb"], data["gpudefrag"]["peak_memory_mb"]),
        "Training Time per Iter (s)": (data["baseline"]["avg_time"], data["gpudefrag"]["avg_time"]),
        "Throughput (iters/s)": (
            data["baseline"].get("throughput_iters_per_sec", 0),
            data["gpudefrag"].get("throughput_iters_per_sec", 0)
        )
    }
    ...
    rects2 = ax.bar(x + width/2, defrag_vals, width, label='gpudefrag', color='#1f77b4')

    ax.set_ylabel('Scores')
    ax.set_title('gpu-defragger: Baseline vs Proactive Defragmentation')
    ax.set_xticks(x)

...
gpudefrag vs vanilla PyTorch on identical workloads.
...
def run_serious_benchmarks(n_trials=5, iterations_per_trial=100):
    ...
    with open(csv_file, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Metric", "Baseline (Mean ± Std)", "gpudefrag (Mean ± Std)", "Improvement"])
    ...
    print("=" * 60)
    print(f"{'Metric':<20} | {'Baseline':<15} | {'gpudefrag':<15} | {'Improvement':<10}")
    print("-" * 60)
    ...
    if MATPLOTLIB_AVAILABLE:
        ...
        rects2 = ax.bar(x + width/2, d_means, width, label='gpudefrag', color='#1f77b4')

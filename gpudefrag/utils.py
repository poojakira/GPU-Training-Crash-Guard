"""
gpudefrag.utils — Shared utilities, logging, and configuration.
"""
...
def get_logger(name: str, level: int = logging.INFO) -> logging.Logger:
    ...
    logger = logging.getLogger(f"gpudefrag.{name}")
...
def ensure_cuda():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA is not available. gpudefrag requires an NVIDIA GPU with CUDA support. "

"""
gpudefrag.collector — High-frequency CUDA allocation trace collector.
...
class AllocationCollector:
    ...
    def start(self) -> None:
        ...
        self._active = True
        self._thread = threading.Thread(target=self._poll_loop, daemon=True, name="gpudefrag-collector")
        self._thread.start()
...
def collect_from_model(model_name: str, iterations: int = 200, config: Optional[DefragConfig] = None) -> int:
    ...
    # Build model
    if model_name == "gpt2":
        from gpudefrag.trainer._models import build_gpt2
        model, inputs = build_gpt2(device)
    elif model_name == "resnet50":
        from gpudefrag.trainer._models import build_resnet50
        model, inputs = build_resnet50(device)
    elif model_name == "bert":
        from gpudefrag.trainer._models import build_bert
